...
bool isSupportedInputName(const char* name) {
    return strcmp(name, "cs40l26_input") == 0 || strcmp(name, "cs40l26_vibra") == 0 ||
...
ndk::ScopedAStatus serviceError(int error) {
    return ndk::ScopedAStatus::fromServiceSpecificError(error < 0 ? -error : error);
}
...
bool Vibrator::openInputLocked() {
    ...
    while ((entry = readdir(dir)) != nullptr) {
        ...
        if (TEMP_FAILURE_RETRY(ioctl(fd, EVIOCGNAME(sizeof(name)), name)) < 0) {
            ALOGD("Failed to read input name for %s: %s", path.c_str(), strerror(errno));
            close(fd);
            continue;
        }
        ...
        if (TEMP_FAILURE_RETRY(ioctl(fd, EVIOCGBIT(EV_FF, sizeof(ffBits)), ffBits)) < 0) {
            ALOGE("Failed to read FF capabilities for %s: %s", path.c_str(), strerror(errno));
            close(fd);
            continue;
        }
        ...
        if (!testBit(FF_PERIODIC, ffBits) ||
            ...
            ALOGE("%s at %s lacks usable force-feedback waveform support", name, path.c_str());
            close(fd);
            continue;
        }
        ...
        ALOGI("Using %s at %s, custom=%d gain=%d sine=%d period=%ums", mInputName.c_str(),
              mInputPath.c_str(), mHasCustom, mHasGain, testBit(FF_SINE, ffBits), kBuzzPeriodMs);
        closedir(dir);
        return true;
    }
...
int Vibrator::playWaveformLocked(uint16_t waveformIndex, int32_t timeoutMs, uint8_t gainPct) {
    ...
    if (TEMP_FAILURE_RETRY(ioctl(mFd, EVIOCSFF, &effect)) < 0) {
        int error = errno;
        ALOGE("Failed to upload RAM waveform %u: %s", waveformIndex, strerror(error));
        return -error;
    }
    ...
    if (written != static_cast<ssize_t>(sizeof(play))) {
        int error = written < 0 ? errno : EIO;
        ALOGE("Failed to start RAM waveform %u effect %d: %s", waveformIndex, mCurrentEffect,
              strerror(error));
        eraseEffectLocked();
        return -error;
    }
...
int Vibrator::playHapticLocked(uint16_t waveformIndex, int32_t timeoutMs, uint8_t gainPct) {
    int ret = playWaveformLocked(waveformIndex, timeoutMs, gainPct);
    if (ret == 0 || ret == -ENODEV) {
        return ret;
    }

    ALOGW("Falling back to FF_SINE for waveform %u after error: %s", waveformIndex,
          strerror(-ret));
    return playSineLocked(timeoutMs, gainPct);
}

void Vibrator::notifyOnComplete(const std::shared_ptr<IVibratorCallback>& callback, int32_t delayMs,
                                uint64_t generation) {
    if (callback == nullptr) {
        return;
    }

    std::thread([this, callback, delayMs, generation] {
...
void Vibrator::scheduleFollowupHaptic(uint64_t generation, int32_t delayMs, uint16_t waveformIndex,
...
uint8_t Vibrator::gainForPrimitive(CompositePrimitive primitive, float scale) const {
    ...
    if (primitive == CompositePrimitive::LIGHT_TICK || primitive == CompositePrimitive::LOW_TICK) {
        if (gain < 45) {
            gain = 45;
        }
    } else if (gain < 30) {
...
ndk::ScopedAStatus Vibrator::perform(Effect effect, EffectStrength strength,
    ...
    uint64_t generation;
    {
        std::lock_guard lock(mLock);
        generation = ++mGeneration;
        int ret = playHapticLocked(waveformIndex,
                                   effect == Effect::DOUBLE_CLICK ? kDoubleClickPulseMs
                                                                  : durationMs,
                                   gainPct);
        if (ret != 0) {
            return serviceError(ret);
...
ndk::ScopedAStatus Vibrator::getSupportedEffects(std::vector<Effect>* _aidl_return) {
    *_aidl_return = {Effect::CLICK, Effect::DOUBLE_CLICK, Effect::TICK, Effect::THUD, Effect::POP,
...
ndk::ScopedAStatus Vibrator::compose(const std::vector<CompositeEffect>& composite,
    ...
    for (const CompositeEffect& effect : composite) {
        ...
        if (effect.primitive != CompositePrimitive::NOOP && primitiveDuration == 0) {
            return unsupported();
        }
